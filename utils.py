...
def validate_function_response(function_response_string: any, strict: bool = False, truncate: bool = True) -> str:
    """Check to make sure that a function used by typeagent returned a valid response
...
def list_agent_config_files(sort="last_modified"):
    ...
    agent_dir = os.path.join(typeagent_DIR, "agents")
...
def list_human_files():
    ...
    defaults_dir = os.path.join(TYPEAGENT_PROJECT_DIR, "humans", "default_humans")
    user_dir = os.path.join(typeagent_DIR, "humans")

    typeagent_defaults = os.listdir(defaults_dir)
    typeagent_defaults = [os.path.join(defaults_dir, f) for f in typeagent_defaults if f.endswith(".txt")]
    ...
    if os.path.exists(user_dir):
        ...
        user_added = []
    return typeagent_defaults + user_added


def list_persona_files():
    """List all personas files"""
    defaults_dir = os.path.join(TYPEAGENT_PROJECT_DIR, "personas", "default_personas")
    user_dir = os.path.join(typeagent_DIR, "personas")
...
def list_systemprompt_files():
    """List all personas files"""
    defaults_dir = os.path.join(TYPEAGENT_PROJECT_DIR, "prompts", "default_system_prompt")
    user_dir = os.path.join(typeagent_DIR, "system_prompts")
