...
from data_sources.connectors import DirectoryConnector
...
def load_file_to_source(server: SyncServer, user_id: uuid.UUID, source: SourceModel, job_id: uuid.UUID, filebytes: bytes,filename:str):
    # update job status
...
def setup_sources_index_router(server: SyncServer, interface: QueuingInterface, password: str):
    ...
    async def attach_source_to_agent(
        ...
        return SourceModel(
            ...
            id=source.id,
            embedding_model=server.server_embedding_config.embedding_model,
    ...
    async def detach_source_from_agent(
        ...
        return JSONResponse(status_code=status.HTTP_200_OK, content={"message": f"Source source_id={source_id} successfully detach from agent "})
    ...
    async def upload_file_to_source(
        ...
        # create job
        job = JobModel(user_id=user_id, metadata_={"type": "embedding", "filename": file.filename, "source_id": str(source_id)},user_status="on")
        job_id = job.id
        server.ms.create_job(job)

        # create background task
        # background_tasks.add_task(load_file_to_source, server, user_id, source, job_id, file)
        jobtask= threading.Thread(target=load_file_to_source, args=(server,user_id,source,job_id,filebytes,filename))
