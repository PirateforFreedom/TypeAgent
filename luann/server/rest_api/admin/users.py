...
def setup_admin_router(server: SyncServer, interface: QueuingInterface):
    ...
    def create_user(request: Optional[CreateUserRequest] = Body(None)):
        ...
        try:
            ...
            if new_user.id==None:
                #  print(str("user_id is none"))
                 newuser_id=server.generate_uuid()
                #  print(str(newuser_id))
                 user_id = uuid.UUID(newuser_id)
                 new_user.id=user_id
            ...
            token = server.ms.create_api_key(user_id=new_user.id, creator="admin")
    ...
    def create_new_api_key(request: CreateAPIKeyRequest = Body(...)):
        ...
        """
        try:
            user = server.ms.get_user(user_id=request.user_id)
            if user is None:
                raise HTTPException(status_code=404, detail=f"User does not exist,can't create a new api key")
            token = server.ms.create_api_key(user_id=request.user_id,creator="admin")
        except HTTPException:
            raise
        except Exception as e:
            raise HTTPException(status_code=500, detail=f"{e}")
