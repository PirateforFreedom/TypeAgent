...
from server.constants import REST_DEFAULT_PORT
from server.rest_api.admin.users import setup_admin_router
from server.rest_api.agents.command import setup_agents_command_router
from server.rest_api.agents.config import setup_agents_config_router
from server.rest_api.agents.index import setup_agents_index_router
from server.rest_api.agents.memory import setup_agents_memory_router
from server.rest_api.agents.message import setup_agents_message_router
from server.rest_api.auth.index import setup_auth_router
from server.rest_api.config.index import setup_config_index_router
from server.rest_api.humans.index import setup_humans_index_router
from server.rest_api.system_prompt.index import setup_systemprompt_index_router

# setup_systemprompt_index_router
# from server.rest_api.interface import QueuingInterface
from server.rest_api.interface import StreamingServerInterface
from server.rest_api.models.index import setup_models_index_router
from server.rest_api.openai_assistants.assistants import (
    setup_openai_assistant_router,
)
from server.rest_api.openai_chat_completions.chat_completions import (
    setup_openai_chat_completions_router,
)
from server.rest_api.personas.index import setup_personas_index_router
from server.rest_api.presets.index import setup_presets_index_router
from server.rest_api.sources.index import setup_sources_index_router
from server.rest_api.static_files import mount_static_files
from server.rest_api.tools.index import setup_tools_index_router
from server.server import SyncServer
from settings import settings

"""
Basic REST API sitting on top of the internal typeagent python server (SyncServer)

Start the server with:
  cd typeagent/server/rest_api
...
app.include_router(setup_systemprompt_index_router(server, interface, password), prefix=API_PREFIX)
...
def on_startup():
    ...
    if app.openapi_schema:
        ...
        app.openapi_schema["info"]["title"] = "TypeAgent API"

    # Split the API docs into typeagent API, and OpenAI Assistants compatible API
    typeagent_api = app.openapi_schema.copy()
    typeagent_api["paths"] = {key: value for key, value in typeagent_api["paths"].items() if not key.startswith(OPENAI_API_PREFIX)}
    typeagent_api["info"]["title"] = "TypeAgent API"
    with open("openapi.json", "w") as file:
        print(f"Writing out openapi.json file")
        json.dump(typeagent_api, file, indent=2)
