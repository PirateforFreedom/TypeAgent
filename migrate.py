...
from data_types import AgentState, Message, Passage, Source, User
...
def wipe_config_and_reconfigure(data_dir: str = typeagent_DIR, run_configure=True, create_config=True):
    """Wipe (backup) the config file, and launch `typeagent configure`"""
    ...
    if run_configure:
        ...
        typeagentConfig.load()


def config_is_compatible(data_dir: str = typeagent_DIR, allow_empty=False, echo=False) -> bool:
    ...
    # typeagent_config = typeagentConfig.load()
    typeagent_config_file = os.path.join(data_dir, "config")
    if not os.path.exists(typeagent_config_file):
        return True if allow_empty else False
    parser = configparser.ConfigParser()
    parser.read(typeagent_config_file)

    if "version" in parser and "typeagent_version" in parser["version"]:
        version = parser["version"]["typeagent_version"]
    else:
        version = None
...
def agent_is_migrateable(agent_name: str, data_dir: str = typeagent_DIR) -> bool:
    ...
    if not hasattr(agent_config, "typeagent_version") or version_less_than(agent_config.typeagent_version, VERSION_CUTOFF):
        return True
    else:
        return False


def migrate_source(source_name: str, data_dir: str = typeagent_DIR, ms: Optional[MetadataStore] = None):
    """
    Migrate an old source folder (`~/.typeagent/sources/{source_name}`).
    """

    # 1. Load the VectorIndex from ~/.typeagent/sources/{source_name}/index
    # TODO
    ...
    try:
        try:
            ...
            if "No module named 'llama_index.schema'" in str(e):
                ...
                raise ValueError(
                    "Failed to load archival memory due thanks to llama_index's breaking changes. Please downgrade to typeagent version 0.3.3 or earlier to migrate this agent."
                )
            else:
                raise e
        ...
        for node in nodes:
            ...
            if len(node.embedding) != config.default_embedding_config.embedding_dim:
                raise ValueError(
                    f"Cannot migrate source {source_name} due to incompatible embedding dimentions. Please re-load this source with `typeagent load`."
                )
            passages.append(
                Passage(
                    user_id=user.id,
        ...
        conn = StorageConnector.get_storage_connector(StorageType.KNOWLEDGE_BASE_PASSAGES, config=config, user_id=user_id)
...
def migrate_agent(agent_name: str, data_dir: str = typeagent_DIR, ms: Optional[MetadataStore] = None) -> List[str]:
    """Migrate an old agent folder (`~/.typeagent/agents/{agent_name}`)
    ...
    try:
        ...
        from openai_backcompat.openai_object import OpenAIObject
    ...
    agent_state = AgentState(
        ...
        preset=agent_config["preset"],  # eg 'typeagent_chat'
        state=dict(
...
def migrate_all_agents(data_dir: str = typeagent_DIR, stop_on_fail: bool = False, debug: bool = False) -> dict:
    ...
    if not config_is_compatible(data_dir, echo=True):
        typer.secho(f"Your current config file is incompatible with typeagent versions >= {VERSION_CUTOFF}", fg=typer.colors.RED)
        if questionary.confirm(
            "To migrate old typeagent agents, you must delete your config file and run `typeagent configure`. Would you like to proceed?"
        ).ask():
            try:
            ...
            typer.secho("Migration cancelled (to migrate old agents, run `typeagent migrate`)", fg=typer.colors.RED)
    ...
    return {
        ...
        "user_id": uuid.UUID(typeagentConfig.load().anon_clientid),
    }


def migrate_all_sources(data_dir: str = typeagent_DIR, stop_on_fail: bool = False, debug: bool = False) -> dict:
    """Scan over all agent folders in data_dir and migrate each agent."""
